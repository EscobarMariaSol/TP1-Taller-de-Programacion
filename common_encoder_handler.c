...
int setEncoder(encoder_handler_t *handler, const char *type, void *key) {
    if (isCesar(type)) {
...
char encoderDecode(encoder_handler_t *handler, char element) {
    if (isCesar(handler->type)) {
